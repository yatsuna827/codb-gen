...
生成の高速化はコード側の大物をほぼ出し切った(全周期走査 2192秒→1584→1274→1242→1222.5→**1,092.8秒**、生成全体 約37分→**18.3分**)。走査は**このCPU(Gracemont/N150)では再抽選バッチのレイテンシ律速**。手書きAVX2化(vmovmskps畳み込み)でクリティカルパスを削り切り、pid%25の逆数乗算・u32レーン読み・movemask畳み込みはいずれも最小命令数に到達している。

残る候補は効果がこのマシンの実測ノイズ以下、または高リスクと判断して見送り:

- **複数軌道のロックステップ・インターリーブ(ILP、見送り)**: 直列レイテンシを隠す最有力手だが、L=4/8/16および手書き4レーンすべてでこのコアでは悪化(詳細は本ドキュメント下部および設計doc)。u16化・AVX2化後もレイテンシ律速の構造は同じで、このチップでは有望性が低い。
- **pid%25の縮約((hi*11+lo)%25、見送り)**: 65536≡11 mod25で`hi*11+lo < 2^20`だが、逆数乗算の丸め正当性の検証が繊細で正のDBを壊すリスク。現行のフルpid%25もLLVM/手書きとも最適逆数乗算で、上積みは不透明。
- **リング折り返しフォールバックの削減(見送り)**: バッチがRING境界(2^18位置に1回)をまたぐ生成でのみ発生し、頻度<0.5%で効果はノイズ以下。
- **PGO / c/n融合 / B・LEAD再掃引(実測して不採用)**: 上記「試したが不採用」参照。
- LEAD縮小・u16化・バッチ境界チェックの高速パス化・状態列ring.s廃止・再抽選バッチの手書きAVX2化は実施済み(上記「生成の高速化(実装済み)」参照)。
