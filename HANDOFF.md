...
**試したが不採用: PGO**。`llvm-tools-preview`で2段PGO(profile-generate→llvm-profdata merge→profile-use、target-cpu=native併用)を試したが素のビルドと測定差なし(律速が分岐予測ミスではなくバッチのレイテンシ鎖である傍証)。ビルドが2段になる割に得が無く不採用。
...
生成の高速化はコード側の大物をほぼ出し切った。走査は**このCPU(Gracemont/N150)では再抽選バッチのレイテンシ律速**で、これを隠す定石のILP(複数軌道のロックステップ・インターリーブ)はこのコアで既に大幅悪化を実測済み(下記)。PGOも測定差なしだった(上記「試したが不採用: PGO」)ことが、分岐ではなくレイテンシが壁であることの傍証。

残る候補は効果がこのマシンの実測ノイズ以下、または高リスクと判断して見送り:

- **複数軌道のロックステップ・インターリーブ(ILP、見送り)**: 直列レイテンシを隠す最有力手だが、L=4/8/16および手書き4レーンすべてでこのコアでは悪化(詳細は本ドキュメント下部および設計doc)。u16化・高速パス化後もレイテンシ律速の構造は同じで、このチップでは有望性が低い。
- **`ring.c`+`ring.n`のu32融合(hop局所性、小粒・未実施)**: hop_keyはimage(約3.8%)でのみ走り、c/nを別配列で2ラインぶん読む。1配列に融合すれば1ラインで済むが、hopは走査全体の少数派で効果は数%未満の見込み。低リスクだが優先度は低い。
...
- LEAD縮小・u16化・バッチ境界チェックの高速パス化は実施済み(上記「生成の高速化(実装済み)」参照)。
